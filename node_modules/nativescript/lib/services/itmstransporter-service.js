...
class ITMSTransporterService {
    ...
    upload(data) {
        return (() => {
            ...
            temp.track();
            let itmsTransporterPath = this.getITMSTransporterPath().wait(), ipaFileName = "app.ipa", itmsDirectory = temp.mkdirSync("itms-"), innerDirectory = path.join(itmsDirectory, "mybundle.itmsp"), ipaFileLocation = path.join(innerDirectory, ipaFileName), loggingLevel = data.verboseLogging ? constants_1.ITMSConstants.VerboseLoggingLevels.Verbose : constants_1.ITMSConstants.VerboseLoggingLevels.Informational, bundleId = this.getBundleIdentifier(data.ipaFilePath).wait(), iOSApplication = this.getiOSApplication(data.username, data.password, bundleId).wait();
            this.$fs.createDirectory(innerDirectory);
            this.$fs.copyFile(data.ipaFilePath, ipaFileLocation);
            let ipaFileHash = this.$fs.getFileShasum(ipaFileLocation, { algorithm: "md5" }).wait(), ipaFileSize = this.$fs.getFileSize(ipaFileLocation), metadata = this.getITMSMetadataXml(iOSApplication.adamId, ipaFileName, ipaFileHash, ipaFileSize);
            this.$fs.writeFile(path.join(innerDirectory, constants_1.ITMSConstants.ApplicationMetadataFile), metadata);
            this.$childProcess.spawnFromEvent(itmsTransporterPath, ["-m", "upload", "-f", itmsDirectory, "-u", helpers_1.quoteString(data.username), "-p", helpers_1.quoteString(data.password), "-v", loggingLevel], "close", { stdio: "inherit" }).wait();
        }).future()();
    }
    getiOSApplications(credentials) {
        return (() => {
            if (!this._itunesConnectApplications) {
                let requestBody = this.getContentDeliveryRequestBody(credentials), contentDeliveryResponse = this.$httpClient.httpRequest({
    ...
    getITMSTransporterPath() {
        return (() => {
            if (!this._itmsTransporterPath) {
                let xcodePath = this.$xcodeSelectService.getContentsDirectoryPath().wait(), xcodeVersion = this.$xcodeSelectService.getXcodeVersion().wait(), result = path.join(xcodePath, "Applications", "Application Loader.app", "Contents");
